...
//  入口：`enum ImgPressAnTool`。使用说明见同目录：`ObjcImgPressAnTool-Swift-README.md`。
//
...
enum ImgPressAnError: Error, Equatable {
    case invalidKBRange(minKB: Int, maxKB: Int)
    case unableToEncode
    case unableToReachTarget
}

/// 压缩结果：`data` 多为 JPEG；`base64` 首次读取时计算并缓存。
final class ImgPressAnOutput: NSObject {
    let data: Data
    let image: UIImage
...
enum ImgPressAnTool {
    ...
    /// - **&gt; 600KB**：压入 **200–600KB**。
    static func compressForUpload200to600(image: UIImage) -> Result<ImgPressAnOutput, ImgPressAnError> {
        let minKB = 200
        let maxKB = 600
        let maxPixel: CGFloat = 4096
        let minLE: CGFloat = 256
        guard let minB = ImgPressAnEngine.byteCount(fromKB: minKB),
              let maxB = ImgPressAnEngine.byteCount(fromKB: maxKB) else {
            return .failure(.invalidKBRange(minKB: minKB, maxKB: maxKB))
        }
        ...
        if j1.count < minB {
            return ImgPressAnEngine.makeCompressOutput(data: j1).map { .success($0) } ?? .failure(.unableToEncode)
        }
        ...
        if j1.count <= maxB, le < minLE,
           let o = ImgPressAnEngine.makeCompressOutputWhenJPEGUnderBudget(
               working: working, jpegData: j1, maxBytes: maxB, longEdgeHardFloor: minLE
           ) { return .success(o) }
        guard var out = ImgPressAnEngine.compressImageToSize(
            image: working,
            targetMinBytes: minB,
            targetMaxBytes: maxB,
        ...
        return ImgPressAnEngine.makeCompressOutput(data: out).map { .success($0) } ?? .failure(.unableToEncode)
    }

    static func compressForUpload200to600Optional(image: UIImage) -> ImgPressAnOutput? {
        try? compressForUpload200to600(image: image).get()
    }

    static func compressForUploadKilobyteRange200to600Async(
        image: UIImage,
        completion: @escaping (Result<ImgPressAnOutput, ImgPressAnError>) -> Void
    ) {
        DispatchQueue.global(qos: .userInitiated).async {
            let r = ImgPressAnTool.compressForUpload200to600(image: image)
...
extension UIImage {
    func ipan_compressForUploadKilobyteRange200to600() -> Result<ImgPressAnOutput, ImgPressAnError> {
        ImgPressAnTool.compressForUpload200to600(image: self)
    }
}

// MARK: - 像素绘制

fileprivate enum ImgPressAnRenderer {
...
fileprivate extension UIImage {
    ...
    func ipan_scaledToMinimumLongEdge(_ minLongEdge: CGFloat) -> UIImage {
        ...
        return ImgPressAnRenderer.render(self, pixelSize: newSize)
    }
...
fileprivate enum ImgPressAnEngine {
    ...
    static func makeCompressOutput(data: Data) -> ImgPressAnOutput? {
        guard let image = UIImage(data: data) else { return nil }
        return ImgPressAnOutput(data: data, image: image)
    }
    ...
    static func makeCompressOutputWhenJPEGUnderBudget(working: UIImage, jpegData: Data, maxBytes: Int, longEdgeHardFloor: CGFloat) -> ImgPressAnOutput? {
        ...
        if bestData == nil || bestData!.count > targetMaxBytes {
            ...
            while let img = resizedImage, lastDataSize > targetMaxBytes {
                ...
                autoreleasepool {
                    ...
                    let newImage = ImgPressAnRenderer.render(img, pixelSize: newSize)
    ...
    static func compressStrictlyUnderMax(image: UIImage, maxBytes: Int, minimumLongEdgeHardFloor: CGFloat) -> Data? {
        ...
        for _ in 0..<64 {
            ...
            autoreleasepool {
                ...
                img = ImgPressAnRenderer.render(img, pixelSize: newSize)
            }
